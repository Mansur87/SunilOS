...
<div class="container-fluid">
	...
	<div class="modal-body">	  
     ...
     <div class="row" style="margin-left:3px;">
			    ...
			    <s:message code="label.searchcourseName" var="coursename"/>
			    <sf:select path="courseId" Class="form-control">
                <sf:option value="0" label="${coursename}" />
				...
				<div class="col-md-3">
			    <div class="form-group">
			   <s:message code="label.searchByExamDate" var="enterdob"></s:message> 
               <sf:input path="examDate" class="form-control" name="dob" id="datepicker" readonly="readonly" placeholder="${enterdob}"/>
               </div>
			...
			<div class="col-md-1"></div>
              ...
              class="page-link btn bg-white btn-block" 
				 ...
				 <%--  <c:if test="${form.pageSize>total.size()}">disabled="disabled"</c:if>
				  <c:if test="${findtotal.size()==form.pageSize*form.pageNo}">disabled</c:if> >
				  --%>
         ...
         <h3 class="success justify-content-center  text-center "><font color="green"> ${success}</font></h3>
          ...
          <c:if test="${!empty list}">
           ...
           <tr>
              ...
              <input type="checkbox" class="form-check-input" name="chk_1" id="sid" onchange="check()"  value="${timetable.id}">
              ...
              <%-- <c:out value="${user.id}" /> --%>
                    ...
                    <td><c:out value="${timetable.courseName}" /></td>
                    <td><c:out value="${timetable.subjectName}" /></td>
                    <td>
                    <fmt:parseDate value="${timetable.examDate}"
							pattern="yyyy-MM-dd" var="myDate" />
						<fmt:formatDate value="${myDate}" var="examDate"
							pattern="MM-dd-yyyy" />
                    <c:out value=" ${examDate}" />
                             
                    </td>
                    <td><c:out value="${timetable.examTime}"/></td>
             
              <td>
              
              <c:url var="editUrl" value="/ctl/TimeTable/TimeTable?id=" />
            
              <a href="${editUrl}${timetable.id}"><s:message code="label.edit"/> <i class="fas fa-edit"></i></a>
