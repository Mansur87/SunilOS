...
/**
 * User functionality Controller. Performs operation for registration,Login
 * ,forgetPassword
 * 
...
public class LoginCtl extends BaseCtl {
	...
	public String display(@ModelAttribute("form") LoginForm form, HttpSession session, Model model, Locale locale) {
		...
		/*
		 * String enteremail = messageSource.getMessage("label.enteremail", null,
		 * locale); model.addAttribute("enteremail", enteremail); String enterpassword =
		 * messageSource.getMessage("label.enterpassword", null, locale);
	...
	public String submit(Locale locale, @ModelAttribute("form") @Valid LoginForm form, BindingResult bindingResult,
		...
		if (OP_SIGNIN.equalsIgnoreCase(form.getOperation())) {
			...
			if (dto != null) {
				...
				System.out.println("form.getUri()--->"+form.getUri());
				if(str==null || str.length()<=0) {
					  return "redirect:/Welcome"; 
					  }
				else {
						return "redirect:"+str.replace("/pro0", ""); 
	 ...
	 * 
	 * @param form:   Object of UserRegistrationForm
	 * @param model:  Object of Model Interface
	 * @param locale: Object of Locale
	 * @return UserRegistration: View of UserRegistration Form
	 */
	...
	public String display(@ModelAttribute("form") UserRegistrationForm form, Model model, Locale locale) {
		...
		/*
		 * String enteremail = messageSource.getMessage("label.enteremail", null,
		 * locale); model.addAttribute("enteremail", enteremail);
		 * 
		 * String enterpassword = messageSource.getMessage("label.enterpassword", null,
		 * locale); model.addAttribute("enterpassword", enterpassword);
		 * 
		 ...
		 * locale); model.addAttribute("enterfirstName", enterfirstName);
		 * 
		 * String enterLastName = messageSource.getMessage("label.enterlname", null,
		 * locale); model.addAttribute("enterLastName", enterLastName);
		 * 
		 * String enterconPassword = messageSource.getMessage("label.entercpassword",
		 * null, locale); model.addAttribute("enterconPassword", enterconPassword);
		 * 
		 * String enterMobile = messageSource.getMessage("label.entermob", null,
		 * locale); model.addAttribute("enterMobile", enterMobile);
		 */
	 ...
	 * 
	 * @param form:   Object of ForgetPasswordForm
	 * @param model:  Object of Model Interface
	 * @param locale: Object of Locale
	 * @return ForgetPassword: View of ForgetPassword Form
	 */
