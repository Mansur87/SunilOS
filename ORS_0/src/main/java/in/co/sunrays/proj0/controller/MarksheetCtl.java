...
import in.co.sunrays.proj0.util.DataValidator;
...
public class MarksheetCtl extends BaseCtl{
	...
	public void preload(Model model) {
		...
		model.addAttribute("marksheetList", list);
	...
	public String display(@RequestParam(required=false) Long id, @ModelAttribute("form") MarksheetForm form, Model model) {
	...
	public String submit(@RequestParam String operation, @ModelAttribute("form") @Valid MarksheetForm form, BindingResult bindingResult, Model model, Locale locale, HttpSession session) {
        ...
        if(OP_SAVE.equalsIgnoreCase(operation)){
	      	 ...
	      	 String maths=String.valueOf(dto.getMaths());
	      	 String physics=String.valueOf(dto.getPhysics());
	      	 String chemistry=String.valueOf(dto.getChemistry());
	      	 if (DataValidator.isNotNull(physics) && !DataValidator.isInteger(String.valueOf(dto.getPhysics()))) {
	      		 String msg=messageSource.getMessage("NotNull",null,locale);
	      		 model.addAttribute("errorphysics",msg);
	      		 return "Marksheet";
	   		 }else if (DataValidator.isNotNull(chemistry) && !DataValidator.isInteger(String.valueOf(dto.getChemistry()))) {
	   			 String msg=messageSource.getMessage("NotNull",null,locale);
		   		 model.addAttribute("errorchemistry",msg);
		   		 return "Marksheet";
		   	 }else if (DataValidator.isNotNull(maths) && !DataValidator.isInteger(String.valueOf(dto.getMaths()))) {
	   			 String msg=messageSource.getMessage("NotNull",null,locale);
	   			 model.addAttribute("errormath",msg);
	   			 return "Marksheet";
	   		 }
	...
	public String displayMarksheet(@ModelAttribute("form") MarksheetForm form, Model model) {
